            ...
            <div className="main-content">
                ...
                <div className="stats-grid">
                    <div className="stat-card">
                        <div className="stat-text">
                            <label>{t('dashboard.my_vehicles')}</label>
                    ...
                    <div className="stat-card">
                        <div className="stat-text">
                            <label>{t('dashboard.next_appointment')}</label>
                    ...
                    <div className="stat-card">
                        <div className="stat-text">
                            <label>{t('dashboard.invoices')}</label>
                ...
                <section className="vehicles-section-main">
                    <div className="vehicles-header-row">
                        <h3>{t('dashboard.my_vehicles')}</h3>
                        <button className="add-vehicle-btn" onClick={handleAddVehicle}>
                            <i className="fa-solid fa-plus"></i> {t('dashboard.add_vehicle')}
                        </button>
                    </div>
                    ...
                    <div className="vehicles-display-container">
                        {vehicles.length === 0 ? (
                            <div className="vehicles-empty-state">
                                ...
                                <p style={{ margin: 0, color: '#64748b' }}>{t('dashboard.no_vehicles')}</p>
                            </div>
                        ) : (
                            <>
                ...
                <section className="repairs-list">
                    <h3>{t('dashboard.my_repairs')}</h3>

                    {loading ? (
                        <div className="loading-container">
                            <i className="fa-solid fa-spinner fa-spin"></i>
                            <p>{t('dashboard.loading_repairs')}</p>
                        </div>
                    ) : repairs.length === 0 ? (
                        <div className="empty-state">
                            <i className="fa-solid fa-inbox"></i>
                            <p>{t('dashboard.no_repairs_found')}</p>
                        </div>
                    ) : (
                        repairs.map(repair => (
                            <div key={repair.id} className="repair-row-card">
                                    ...
                                    <div className="repair-info">
                                        <div className="service-tags">
                                            {repair.services && repair.services.length > 0 ? (
                                                ...
                                                <span className="tag">{t('dashboard.general_service')}</span>
                                            )}
                                        </div>
                                        <h4>{repair.vehicle?.make} {repair.vehicle?.model}</h4>
                                        <p className="due-date">{t('dashboard.due')}: {repair.date_end ? new Date(repair.date_end).toLocaleDateString('en-GB', {
                                            year: 'numeric',
                                            month: 'short',
                                            day: 'numeric'
                                ...
                                <div className="repair-actions">
                                    {repair.status?.toLowerCase() === 'completed' && (
                                        <>
                                            <span className="status-label completed-label">{t('dashboard.reduction_requested')}</span>
                                    ...
                                    {repair.status?.toLowerCase() === 'estimate sent' && (
                                        <>
                                            <span className="status-label ready-label">{t('dashboard.estimate_ready')}</span>
                                            <button className="btn-secondary" onClick={() => handleDownloadEstimate(repair)}>
                                                {t('dashboard.download_estimate')}
                                            </button>
                                            <button
                                            ...
                                            >
                                                {actionLoading === repair.id ? <i className="fa-solid fa-spinner fa-spin"></i> : t('dashboard.accept')}
                                            </button>
                                            <button
                                            ...
                                            >
                                                {actionLoading === repair.id ? <i className="fa-solid fa-spinner fa-spin"></i> : t('dashboard.request_reduction')}
                                            </button>
                                        </>
                                    )}
                                    {repair.status?.toLowerCase() === 'negotiation requested' && (
                                        <>
                                            <span className="status-label pending-label">{t('dashboard.negotiation_pending')}</span>
                                            <button className="btn-secondary" onClick={() => handleDownloadEstimate(repair)}>
                                                {t('dashboard.download_estimate')}
                                            </button>
                                            <button className="btn-primary" disabled={actionLoading === repair.id} onClick={() => handleApproveJob(repair.id)}>
                                                {actionLoading === repair.id ? <i className="fa-solid fa-spinner fa-spin"></i> : t('dashboard.approve')}
                                            </button>
                                            <button className="btn-primary" disabled>{t('dashboard.request_reduction')}</button>
                                        </>
                                    )}
                                    {repair.status?.toLowerCase() === 'in progress' && (
                                        <span className="status-label progress-label">{t('dashboard.in_progress')}</span>
                                    )}
                                    {repair.status?.toLowerCase() === 'pending' && (
                                        <span className="status-label pending-label">{t('dashboard.estimate_requested')}</span>
                                    )}
                                </div>
                            </div>
                        ))
            ...
            {showAddVehicleModal && (
                <div className="modal-overlay" onClick={handleCloseVehicleModal}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        <h2>{t('dashboard.modal_add_title')}</h2>
                        <form onSubmit={handleSubmitVehicle}>
                            <div className="form-group">
                                <label>{t('dashboard.modal_maker')} <span className="required">*</span></label>
                                <input
                                    type="text"
                                    placeholder={t('dashboard.modal_maker_placeholder')}
                            ...
                            <div className="form-group">
                                <label>{t('dashboard.modal_model')} <span className="required">*</span></label>
                                <input
                                    type="text"
                                    placeholder={t('dashboard.modal_model_placeholder')}
                            ...
                            <div className="form-group">
                                <label>{t('dashboard.modal_license_plate')} <span className="required">*</span></label>
                                <input
                                    type="text"
                                    placeholder={t('dashboard.modal_plate_placeholder')}
                            ...
                            <div className="form-group">
                                <label>{t('dashboard.modal_year')} <span className="required">*</span></label>
                                <input
                                    type="number"
                            ...
                            <div className="form-group">
                                <label>{t('dashboard.modal_type')} <span className="required">*</span></label>
                                <select
                                    value={newVehicle.type}
                                    onChange={(e) => setNewVehicle({ ...newVehicle, type: e.target.value })}
                                    required
                                >
                                    <option value="car">{t('dashboard.modal_car')}</option>
                                    <option value="bus">Bus</option>
                                    <option value="truck">{t('dashboard.modal_truck')}</option>
                                    <option value="moto">{t('dashboard.modal_motorcycle')}</option>
                                </select>
                            </div>
                            <div className="modal-actions">
                                ...
                                >
                                    {submittingVehicle ? (
                                        <>
                                            <i className="fa-solid fa-spinner fa-spin"></i> {t('dashboard.modal_adding')}
                                        </>
                                    ) : (
                                        t('dashboard.modal_add_button')
                                    )}
                                </button>
                            </div>
                        </form>
