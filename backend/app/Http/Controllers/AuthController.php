...
class AuthController extends Controller
{
    ...
    public function verifyemail(Request $request)
    {
        ...
        if ($user->is_verified) {
            return response()->json(['message' => 'auth.already_verified'], 400);
        }
        ...
        if (!$otpRecord) {
            return response()->json(['message' => 'auth.no_otp_found'], 404);
        }

        // Check if expired
        if ($otpRecord->isExpired()) {
            $otpRecord->delete();
            return response()->json(['message' => 'auth.otp_expired'], 400);
        }

        // Validate OTP
        if (!$otpRecord->isValid($request->otp)) {
            return response()->json(['message' => 'auth.invalid_otp'], 400);
        }
    ...
    public function login(Request $request)
    {
        ...
        if (!$user || !Hash::check($fields['password'], $user->password)) {
            return response()->json(['message' => 'auth.bad_credentials'], 401);
        }
